...
// the response object, aand the next middleware function in the app's request-response cycle.
function authenticateToken(req, res, next) {
  ...
  // it splits the header value by spaces aand takes the second part, which should be the token itself.
  const token = authHeader && authHeader.split(" ")[1];
  // This line checks if the token is not present. If there is no token,
  // it sends a 401 Unthorized status code as the response,
